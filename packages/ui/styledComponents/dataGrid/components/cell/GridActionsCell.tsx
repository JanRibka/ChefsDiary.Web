...
interface GridActionsCellProps
  extends Omit<GridRenderCellParams, "value" | "formattedValue"> {
...
const GridActionsCell = (props: GridActionsCellProps) => {
  ...
  const ignoreCallToFocus = useRef(false);
  const touchRippleRefs = useRef<Record<string, TouchRippleActions | null>>({});
  const menuId = useId();
