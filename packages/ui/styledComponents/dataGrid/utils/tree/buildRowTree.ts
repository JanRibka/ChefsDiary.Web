...
} from "../../hooks/features/rows/gridRowState";
...
interface TempRowTreeNode
  extends Omit<GridRowTreeNodeConfig, "children" | "childrenExpanded"> {
  ...
  for (let i = 0; i < ids.length; i += 1) {
    ...
    const nodeWithoutChildrenExpansion: GridRowTreeNodeConfig = {
      ...
      children: tempNode.children
        ? Object.values(tempNode.children)
        : undefined,
    };
    const node = {
    ...
    const shouldReUsePreviousNode =
      ...
      previousNode.children?.every(
        (childId, index) => node.children?.[index] === childId
      );
