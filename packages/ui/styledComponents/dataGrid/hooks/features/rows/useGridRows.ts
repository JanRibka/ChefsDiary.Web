...
import { GridRowsInternalCache } from "./gridRowState";
import {
...
export const useGridRows = (
  ...
  const getRow = useCallback<GridRowApi["getRow"]>(
  ...
  const throttledRowsChange = useCallback(
   ...
   * API METHODS
   */
  const setRows = useCallback<GridRowApi["setRows"]>(
    (rows) => {
  ...
  const updateRows = useCallback<GridRowApi["updateRows"]>(
    (updates) => {
      ...
      const newCache: GridRowsInternalCache = {
        ...
        loadingPropBeforePartialUpdates:
          prevCache.loadingPropBeforePartialUpdates,
        rowCountPropBeforePartialUpdates:
          prevCache.rowCountPropBeforePartialUpdates,
  ...
  const getRowModels = useCallback<GridRowApi["getRowModels"]>(() => {
  ...
  const getRowsCount = useCallback<GridRowApi["getRowsCount"]>(
    () => gridRowCountSelector(apiRef),
    [apiRef]
  );

  const getAllRowIds = useCallback<GridRowApi["getAllRowIds"]>(
    () => gridRowIdsSelector(apiRef),
    [apiRef]
  );

  const getRowIndexRelativeToVisibleRows = useCallback<
    GridRowApi["getRowIndexRelativeToVisibleRows"]
  >((id) => lookup[id], [lookup]);

  const setRowChildrenExpansion = useCallback<
    GridRowApi["setRowChildrenExpansion"]
  >(
    (id, isExpanded) => {
      ...
      const newNode: GridRowTreeNodeConfig = {
        ...currentNode,
  ...
  const getRowNode = useCallback<GridRowApi["getRowNode"]>(
    (id) => gridRowTreeSelector(apiRef)[id] ?? null,
    [apiRef]
  );

  const getRowGroupChildren = useCallback<GridRowApi["getRowGroupChildren"]>(
    ({
  ...
  const setRowIndex = useCallback<GridRowApi["setRowIndex"]>(
    (rowId, targetIndex) => {
      const allRows = gridRowIdsSelector(apiRef);
  ...
  const replaceRows = useCallback<GridRowApi["unstable_replaceRows"]>(
    (firstRowToRender, newRows) => {
      ...
      newRowEntries.forEach((row, index) => {
        const [replacedRowId] = updatedRows.splice(
          firstRowToRender + index,
          1,
          row.id
        );
  ...
  const groupRows = useCallback(() => {
    ...
    if (
      apiRef.current.unstable_caches.rows.rowsBeforePartialUpdates ===
      props.rows
    ) {
  ...
  const handleStrategyProcessorChange = useCallback<
  ...
  const handleStrategyActivityChange = useCallback<
   ...
   * APPLIERS
   */
  const applyHydrateRowsProcessor = useCallback(() => {
    apiRef.current.setState((state) => ({
      ...state,
      rows: {
        ...state.rows,
  ...
  useEffect(() => {
    ...
    const isNewLoadingAlreadyInState =
      apiRef.current.unstable_caches.rows.loadingPropBeforePartialUpdates ===
      props.loading;
    const isNewRowCountAlreadyInState =
      apiRef.current.unstable_caches.rows.rowCountPropBeforePartialUpdates ===
      props.rowCount;
