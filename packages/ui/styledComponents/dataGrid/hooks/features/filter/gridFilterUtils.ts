  ...
  return (rowId, shouldApplyFilter) => {
    ...
    const filteredAppliers = shouldApplyFilter
      ? appliers.filter((applier) =>
          shouldApplyFilter(applier.item.columnField)
        )
      : appliers;
  ...
  return (rowId, shouldApplyFilter) => {
    ...
    Object.keys(appliersPerColumnField).forEach((columnField) => {
      if (!shouldApplyFilter || shouldApplyFilter(columnField)) {
        usedCellParams[columnField] = apiRef.current.getCellParams(
          rowId,
          columnField
        );
...
export const passFilterLogic = (
  ...
  const linkOperator =
    filterModel.linkOperator ?? getDefaultGridFilterModel().linkOperator;

  // get result for filter items model
  if (cleanedAllFilterItemResults.length > 0) {
    // Return true if the item pass with one of the rows
    const filterItemPredicate = (item: GridFilterItem) => {
      return cleanedAllFilterItemResults.some(
  ...
  if (
    cleanedAllQuickFilterResults.length > 0 &&
