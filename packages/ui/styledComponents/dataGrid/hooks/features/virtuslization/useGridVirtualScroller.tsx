...
import { gridEditRowsStateSelector } from "../editRow/gridEditRowsSelector";
import {
...
export const useGridVirtualScroller = (props: UseGridVirtualScrollerProps) => {
  ...
  const renderZoneRef = useRef<HTMLDivElement>(null);
  const rootRef = useRef<HTMLDivElement>(null);
  const handleRef = useForkRef(ref, rootRef);
  const [renderContext, setRenderContext] = useState<GridRenderContext | null>(
    null
  );
  const prevRenderContext = useRef<GridRenderContext | null>(renderContext);
  ...
  const prevTotalWidth = useRef(columnsTotalWidth);

  const getNearestIndexToRender = useCallback(
    (offset: number) => {
  ...
  const computeRenderContext = useCallback(() => {
  ...
  const handleResize = useCallback<GridEventListener<"resize">>((params) => {
    setContainerDimensions({
      width: params.width,
  ...
  const updateRenderZonePosition = useCallback(
  ...
  const getRenderContext = useCallback((): GridRenderContext => {
