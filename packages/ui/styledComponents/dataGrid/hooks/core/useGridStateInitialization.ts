...
export const useGridStateInitialization = <Api extends GridApiCommon>(
  ...
  const controlStateMapRef = useRef<
    Record<string, GridControlStateItem<Api["state"], any>>
  >({});
  const [, rawForceUpdate] = useState<Api["state"]>();

  const registerControlState = useCallback<
  ...
  const setState = useCallback<GridStateApi<Api["state"]>["setState"]>(
    (state, reason) => {
